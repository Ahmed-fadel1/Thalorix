...
class PostModel {
  ...
  factory PostModel.fromJson(Map<String, dynamic> json) {
    ...
    if (user != null && user['name'] != null) {
      ...
      resolvedAvatar = user['avatar'] ?? user['profilePic'] ?? user['photo'] ?? user['image'] ?? user['profilePicture'];
    } else if (json['authorName'] != null && json['authorName'].toString().isNotEmpty) {
      // Backend returned authorName as a separate field
      resolvedName = json['authorName'];
      resolvedAvatar = json['authorAvatar'] ?? json['authorProfilePic'] ?? json['authorPhoto'];
    } else if (json['userName'] != null && json['userName'].toString().isNotEmpty) {
      // Backend returned userName as a separate field
      resolvedName = json['userName'];
      resolvedAvatar = json['userAvatar'] ?? json['userProfilePic'] ?? json['userPhoto'];
    } else if (json['name'] != null && json['name'].toString().isNotEmpty) {
      // Backend returned name at top level
      resolvedName = json['name'];
      resolvedAvatar = json['avatar'] ?? json['profilePic'] ?? json['photo'];
    } else {
      // Backend returned userId as plain string with no name info
      ...
      if (currentUserId != null && currentUserId == rawUserId) {
        ...
        resolvedAvatar = CacheHelper.getProfilePic(); // Use cached avatar!
      } else {
